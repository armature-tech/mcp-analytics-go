...
const project = await mkdtemp(join(tmpdir(), "sdk-canary-go-vercel-"));
try {
  await cp(join(root, "canary", "vercel"), project, { recursive: true });
  const candidate = join(project, "candidate");
  await mkdir(candidate, { recursive: true });
  for (const entry of ["go.mod", "go.sum", "armatureanalytics"]) {
    await cp(join(root, entry), join(candidate, entry), { recursive: true });
  }
  ...
  run("go", ["test", "./..."], { cwd: project });
  const output = runVercel([
    "deploy", "--prod", "--skip-domain", "--force", "--yes", "--scope", scope, ...authArgs,
