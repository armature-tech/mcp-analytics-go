...
const arg = name => { const index = process.argv.indexOf(name); if (index < 0 || !process.argv[index + 1]) throw new Error(`missing ${name}`); return process.argv[index + 1]; };
...
const dispatchResponse = await fetch(`${base}/api/internal/sdk-canary-dispatch`, { method: "POST", headers: { authorization: `Bearer ${dispatchSecret}`, "content-type": "application/json" }, body: JSON.stringify({ package: packageName, marker }) });
...
const harnessFamily = session => { const client = String(session.client_name || "").trim().toLowerCase(); if (client === "mcp-tester-claude-remote-proxy" || /claude[ _-]*code/.test(client)) return "claude_code"; if (/codex/.test(client)) return "codex"; return `unexpected:${client || "unknown"}`; };
